...
function uptime() {
	$.getJSON("json/stats.json", function(result) {
		...
		for (var i = 0; i < result.servers.length; i++) {
			...
			//	TableRow.children["online6"].children[0].children[0].innerHTML = "<small>在线</small>";
			//} else {
			...
			if (!result.servers[i].online4 && !result.servers[i].online6) {
				...
				ExpandRow[0].children["expand_mem"].innerHTML = "Memory Info: " + bytesToSize(result.servers[i].memory_used*1024, 2) + " / " + bytesToSize(result.servers[i].memory_total*1024, 2);
				// Swap
				ExpandRow[0].children["expand_swap"].innerHTML = "Swap Info: " + bytesToSize(result.servers[i].swap_used*1024, 2) + " / " + bytesToSize(result.servers[i].swap_total*1024, 2);
				...
				ExpandRow[0].children["expand_hdd"].innerHTML = "Disk Info: " + bytesToSize(result.servers[i].hdd_used*1024*1024, 2) + " / " + bytesToSize(result.servers[i].hdd_total*1024*1024, 2);
